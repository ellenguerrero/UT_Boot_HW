...
SELECT COUNT(film_id) AS 'TOTAL COPIES OF HUNCHBACK IMPOSSIBLE' FROM inventory WHERE film_id 
IN(SELECT film_id FROM film WHERE title="Hunchback Impossible");

-- 6e. Using the tables payment and customer and the JOIN command, list the total paid by each customer. 
-- List the customers alphabetically by last name:

SELECT customer.customer_id, customer.first_name, customer.last_name, SUM(amount) AS 'TOTAL PAID BY CUSTOMER'
FROM customer
JOIN payment
...
SELECT title, language_id FROM film WHERE title LIKE 'K%' OR '%Q' 
...
-- SELECT first_name, last_name, email FROM customer WHERE address_id
-- IN(SELECT address_id FROM address WHERE city_id
-- IN(SELECT city_id FROM city WHERE country_id
-- IN (SELECT country_id FROM country WHERE country_id=20)));
...
SELECT rental.rental_id, film.title, COUNT(rental.rental_date) AS 'Number of Rentals'
FROM film 
JOIN inventory 
...
SELECT store.store_id, SUM(amount) AS 'TOTAL AMOUNT BY STORE'
FROM store 
